    ...
    void runIncr(T& command, std::function<void(int, const BSONObj&)> postAssert) {
        ...
        for (std::int32_t i : {123, 12345, 0, -456}) {
            ...
            const BSONObj reply = [&] {
                ...
                try {
                    invocation->run(opCtx.get(), replyBuilder.get());
                ...
                rpc::OpMsgReplyBuilder* opReplyBuilder =
                    static_cast<rpc::OpMsgReplyBuilder*>(replyBuilder.get());
                invariant(opReplyBuilder);
                return BSONObj(opReplyBuilder->release());
            }();
