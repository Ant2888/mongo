...
public:
    ...
    private:
        ...
        NamespaceString ns() const override {
            return GetMoreRequest::parseNs(_dbName, _request.body);
        }

        void doCheckAuthorization(OperationContext* opCtx) const override {
            uassertStatusOK([&] {
                StatusWith<GetMoreRequest> parseStatus =
                    GetMoreRequest::parseFromBSON(_dbName, _request.body);
                if (!parseStatus.isOK()) {
        ...
        bool runParsed(OperationContext* opCtx,
            ...
            ScopeGuard cursorFreer =
                MakeGuard(&ClientCursorPin::deleteUnderlying, &ccPin.getValue());
            ...
            Status batchStatus =
                generateBatch(opCtx, cursor, request, &nextBatch, &state, &numResults);
        ...
        void run(OperationContext* opCtx, rpc::ReplyBuilderInterface* reply) override {
            ...
            StatusWith<GetMoreRequest> parsedRequest =
                GetMoreRequest::parseFromBSON(_dbName, _request.body);
            ...
            try {
                auto ok = runParsed(opCtx, request.nss, request, this->_request.body, bob);
                CommandHelpers::appendSimpleCommandStatus(bob, ok);
            } catch (const ExceptionFor<ErrorCodes::Unauthorized>& e) {
                CommandHelpers::auditLogAuthEvent(opCtx, this, this->_request, e.code());
                throw;
            }
        }
